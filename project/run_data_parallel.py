...
def average_gradients(model):
    ...
    if size <= 1:
        return
    # yeah we just smash the grads together and split them back out
...
def setup(rank, world_size, backend):
    ...
    os.environ.setdefault('MASTER_PORT', '11868')
    # nothing fancy, just bring the gang online
    dist.init_process_group(backend=backend, rank=rank, world_size=world_size)
    if torch.cuda.is_available():
        try:
            torch.cuda.set_device(rank % torch.cuda.device_count())
        except Exception:
            pass  # idk, sometimes cpu only runs
    # END ASSIGN5_1_2
...
def run_dp(
    ...
    # tear down the party so repeated runs don't explode
